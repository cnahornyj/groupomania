<template>
    <main>
        <form method="POST" v-on:submit.prevent="create()">
            <div class="group">
                <input type="text" name="title" aria-label="Contenu du post" class="title" v-model="content" placeholder="Ecrivez le contenu de votre post...">
                <span v-if="(!$v.content.required && $v.content.$dirty) && submited" class="error-message">Veuillez ajouter un titre</span><br>
            </div>
            <div class="group">
                <input type="file" name="image" aria-label="Fichier à sélectionner" class="file" @change="onFileSelected" hidden>
...
export default {
    name: 'CreatePosts',
    props:['posts', 'role', 'printNewPosts'],
    data(){
        return{
            content: '',
            selectedFile: null,
